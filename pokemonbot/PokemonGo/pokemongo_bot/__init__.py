...
data_pokemon= join(dirname,"data/pokemon.json")
data_items =join(dirname,"data/items.json")

class PokemonGoBot(object):
    def __init__(self, config):
        self.config = config
        self.pokemon_list = json.load(open(data_pokemon))
        self.item_list = json.load(open(data_items))
    ...
    def work_on_cell(self, cell, position, include_fort_on_path):
        ...
        if (self.config.mode == "all" or self.config.mode ==
            ...
            user_web_catchable=join(dirname,user_web_catchable)
    ...
    def _setup_api(self):
        ...
        try:
            release_config=join(dirname,'release_config.json')
            with open(release_config) as file:
                pass
        except:
    ...
    def pokeball_inventory(self):
        ...
        user_web_inventory=join(dirname,user_web_inventory)
    ...
    def _set_starting_position(self):
        ...
        if self.config.location_cache and not self.config.location:
            try:
                ...
                # the location.json
                last_location_path='data/last-location-%s.json' % (self.config.username)
                last_location_path=join(dirname,last_location_path)
                with open(last_location_path) as f:
